...
class SiteMapController extends Controller
{
    ...
    {
        $this->custom_links = ['categories', 'tags', 'articles'];
        $this->data = collect([
            [
                'name'             => "articles",
                'index_route_name' => "article.index",
                'show_route_name'  => "article.show",
                'helper_function'  => "redirectArticleRoute",
                'data'             => \App\Models\Article::query(),
            ],
            [
                'name'             => "categories",
                'index_route_name' => "category.index",
                'show_route_name'  => "category.show",
                'helper_function'  => "redirectCategoryRoute",
                'data'             => \App\Models\Category::query(),
            ],
            [
                'name'             => "tags",
                'index_route_name' => "tag.index",
                'show_route_name'  => "tag.show",
                'helper_function'  => "redirectTagRoute",
                'data'             => \App\Models\Tag::query(),
            ],
        ]);
    }
    ...
    {
        $request->merge(['name' => $name, 'page' => $page]);
        $request->validate([
        ...
        $route = $this->data->where('name', $request->name)->first()['helper_function'];
        foreach ($items as $item) {
            $url = '<url><loc>' . /*str_replace('&', '-', $route($item))*/ $route($item). '</loc><priority>1.0</priority><lastmod>' . gmdate(DateTime::W3C, strtotime($item->updated_at)) . '</lastmod></url>';
            array_push($urls, $url);
        }
        $urls = implode('', $urls);
